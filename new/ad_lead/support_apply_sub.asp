...
IF isArray(ArySelection) Then
	IF regyear = "" or UBound(ArySelection) < 1 Then
...
Function GetPartListToArray(ByVal regyear)
	...
	'sql = sql & " on convert(char(10),getdate(),23) between s.From_Date and s.To_Date AND c.Study_year = s.Study_year"
	sql = sql & " on '" & regyear & "-"& dd2 & "-" & dd3 & "'" & " between s.From_Date and s.To_Date AND c.Study_year = s.Study_year"
...
<input type="hidden" name="interviewconfirm" value="주1회" />
...
<h3 class="title02">학년 선택</h3>
	...
	<tbody>
		<tr>
            ...
            <td class="txtLeft">
            	<label><input type="radio" name="checkyear" <%IF StrComp(regyear,2022) = 0 Then %>checked="checked"<%END IF%> value="2022"> 2022학년도 </label>
<!--				<label ><input type="radio" name="checkyear" <%IF StrComp(regyear,2021) = 0 Then %>checked="checked"<%END IF%> class="ml15" value="2021"> 2021학년도</label>-->
            </td>
        </tr>
    </tbody>
    </table>
	...
	<tbody>
		<tr>
			...
			<td class="txtLeft">
                ...
                <label ><input type="radio" name="interviewmemo"  class="ml15" checked value="[재직자/특성화고 전형]"> 재직자/특성화고 전형</label>
...
<input type="hidden" name="part1" id="part1" class="input-part" value="" />
<select name="course1" id="course1" class="select-part-course">
