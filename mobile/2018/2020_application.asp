...
'response.write "regyear : " & regyear
...
function check_theForm() {
	...
	with(document.theForm) {
        ...
        if (!$('input:radio[name=checkyear]').is(':checked')) {
			alert( "학년도구분을 선택해 주세요" );
			unCheckObj("checkyear");
			return;
		}
		...
		/*if (address1.selectedIndex == 0) {
			alert ("지역을 반드시 선택해 주세요");
			unCheckObj("address1");
			//address1.focus();
			return;
		}*/
		/*
		val = $("#parent_tel1").val().num();
		$("#parent_tel1").val(val);
		if (val.length < 9) {
			alert ("보호자연락처를 입력해 주세요");
			parent_tel1.focus();
			return;
		}
		*/
		...
		if (!$('input:radio[name=Imotivation]').is(':checked')) {
			alert( "지원경로를 선택해 주세요" );
			unCheckObj("ImotivationWrap");
			return;
		}

		if (!$('input:radio[name=recommend2]').is(':checked')) {
			alert( "면접유형을 선택해 주세요" );
			unCheckObj("recommend2");
			return;
        }
		...
		<section class="sub_in_wrap">
			...
			<form name="theForm" id="theForm" method="post">
				<div class="entrance application" data-role="content" data-enhance="false">
					...
					<h3 class="title">학년 선택</h3>
					<table class="contentsTable inputForm">
						<caption>
						<strong>학년 선택</strong>
						<p>고3(21학번), 예비 고3(22학번) 선택</p>
						</caption>
						<colgroup>
						<col width="26%">
						<col width="*">
						...
						<tbody>
							<tr>
								<th class="txtLeft">학년<span class="star">*</span></th>
								<td class="txtLeft">
									<ul class="yearWrap">
										<li>
											<label>
												<!-- <input type="radio" class="ml10" name="checkyear" <%IF StrComp(regyear,"2021") = 0 Then %>checked="checked"<%END IF%>  value="2021">2021학년도</label> -->
												<label><input type="radio" class="ml10" id="checkyear" name="checkyear" <%IF StrComp(regyear,"2022") = 0 Then %>checked="checked"<%END IF%>  value="2022">2022학년도</label>
										</li>
									</ul>
								</td>
							</tr>
				  ...
				  '	END IF
					END IF
					%>
					<h3 class="title">지원 전형 및 계열 선택</h3>
					...
					<table class="contentsTable inputForm" summary="신입생 원서접수 지원자 인적사항">
						...
						<tbody>
								...
								<td class="txtLeft">
									<ul class="courseWrap">
										<li>
											...
											PartListArray = GetPartListToArray(regyear)
                                            CourseListArray = GetCourseListToArray(regyear)

											PartListArray2 = GetPartListToArray2(regyear)
                                            CourseListArray2 = GetCourseListToArray2(regyear)
							...
							<tr>
								...
								<td class="txtLeft">
									<label>
										<input type="radio" name="interviewmemo" checked="checked" value="[일반전형]">
										일반전형</label>
									<label >
										<input type="radio" name="interviewmemo" value="[특별전형]">
										특별전형</label>
									<label >
										<input type="radio" name="interviewmemo" value="[편입학전형]">
										편입학전형</label>
								</td>
							</tr>
						</tbody>
					</table>
					<h3 class="title">지원자 정보</h3>
					<table class="contentsTable inputForm" summary="신입생 원서접수 지원자 인적사항">
						<caption>
						신입생 원서접수 지원자 인적사항
						</caption>
						...
						<tbody>
							...
							<tr>
								<th class="txtLeft">
									<legend>성별<span class="star">*</span></legend>
								</th>
								<td class="txtLeft">
									<input type="radio" name="sex" id="sex_1" checked="checked" value="1" />
									<label for="sex_1">남</label>
									<input type="radio" name="sex" id="sex_2" value="2" />
									<label for="sex_2">여</label>
								</td>
							</tr>
							<tr>
								<th class="txtLeft">
								...
								<td class="txtLeft">
									...
									<input name="mobile2" type="tel" id="mobile2" maxlength="4" class="w20 mr15"placeholder="휴대폰 가운데번호를 입력해주세요." onKeyPress="onlyNumber(event);" />
									-
									<label for="mobile3"></label>
									<input name="mobile3" type="tel" id="mobile3" maxlength="4" class="w20"  placeholder="휴대폰 뒷번호를 입력해주세요." onKeyPress="onlyNumber(event);" />
								</td>
							</tr>
							<!--<tr>
								<th class="txtLeft">
									<label for="address1">지역<span class="star">*</span></label>
								</th>
								<td>
									<% addr_area_array = Split("서울특별시,경기도,인천광역시,강원도,충청남도,대전광역시,충청북도,세종특별자치시,부산광역시,울산광역시,대구광역시,경상북도,경상남도,전라남도,광주광역시,전라북도,제주특별자치도", ",") %>
									<% addr_zipcode_array = Split("010XX,100XX,210XX,240XX,310XX,340XX,270XX,300XX,460XX,440XX,410XX,360XX,500XX,570XX,610XX,540XX,630XX", ",") %>
									<select name="address1" id="address1" class="address">
										<option value="">지역선택</option>
										<%
										For i = 0 To Ubound(addr_area_array,1)
											Response.Write "<option value=""" & addr_zipcode_array(i) & "|" & addr_area_array(i) & """>" & addr_area_array(i) & "</option>"
										Next
										%>
									</select>
								</td>
               ...
               <tr>
				...
				<td class="txtLeft">
					<input name="zipcode" type="text" id="zipcode" readonly class="w40" maxlength="7" title="우편번호" />
					<a href="#" title="우편번호검색" class="btnZip btnDouble mb05" id="zip_btn">우편번호 검색</a>
					<br>
					<input name="address1" type="text" id="address1" class="w80 mt05" readonly  maxlength="100"/><br><input name="address2" type="text" id="address2" class="ml15 w80 mt05" maxlength="100" title="상세주소입력" />
				</td>
			</tr>
					...
					<table class="contentsTable inputForm" summary="신입생 원서접수 지원자 인적사항">
						...
						<tbody>
							<tr>
								<th rowspan="2" class="txtLeft">
									<label for="Hname">고등학교<span class="star">*</span></label>
								</th>
								<td class="txtLeft" colspan="2">
									<input id="Hname" name="Hname" value="" type="text" placeholder="고등학교를 검색해주세요." maxlength="100"  />
									<input name="Hcode" type="hidden" />
									<input name="Harea" type="hidden" />
									<br>
									<!--<a href="#Sfrm" class="btnZip btnSearch mr10" style="padding:0px; font:inherit;"  id="school_btn">고등학교검색</a>-->
									&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="http://www.koreait.ac.kr//new/ad_lead/images/한국IT전문학교교사추천서.hwp" title="추천서 다운로드" class="btnZip btnPost mr10">추천서 다운로드</a> </td>
							<!--<tr>
								<th class="txtLeft wid25">
									<label for="Hgdate1">졸업(예정)년도<span class="star">*</span></label>
								</th>
								<td class="txtLeft">
									<ul class="HgdateWrap">
										<li>
											<select name="Hgdate1" id="Hgdate1" >
												<option value="">졸업(예정)년도</option>
												<%
										...
										<li>
											<label for="Hgdate2"></label>
											<select name="Hgdate2" id="Hgdate2" >
												<%
												For i = 1 To 12
                ...
                <ul class="HgdateWrap">
                    ...
                    <li>
                        ...
                        <select name="Hgdate2" id="Hgdate2" >
							...
							<tr>
								<th rowspan="2" class="txtLeft">
									<label for="ocs_status">타대학 지원현황</label>
								</th>
								<td colspan="3" class="txtLeft">
									<label>
										<input type="radio" name="ocs_status" value="" checked>
										<strong class="point">없음</strong></label>
									<label>
										<input type="radio" name="ocs_status" value="1">
										<strong class="point">대학교</strong></label>
									<label>
										<input type="radio" name="ocs_status" value="2">
										<strong class="point">전문대학</strong></label><br>
									<label>
										<input type="radio" name="ocs_status" value="3">
										<strong class="point">전문학교</strong></label>
									<label>
										<input type="radio" name="ocs_status" value="4">
										<strong class="point">평생교육원</strong></label>
								</td>
							</tr>
							<tr>
								<td colspan="2" class="txtLeft">
									<input name="ocs_name" type="text" id="ocs_name" class="mb10 w70 input-disabled"  maxlength="100" disabled="disabled" />
									대학교
									<input name="ocs_major" type="text" id="ocs_major" class="w70 input-disabled" maxlength="15" title="전공" disabled="disabled" />
									전공 </td>
							</tr>
							<tr>
								<th class="txtLeft">학원</th>
								<td class="txtLeft" colspan="3">
									<input type="text" name="academy" id="academy" class="w70" maxlength="100" value=""/>
									<br>
									<span>※ 현재 학원을 다니는 경우에만 입력하세요!</span> </td>
							</tr>
						</tbody>
					</table>
					<h3 class="title">보호자 인적사항</h3>
					<table class="contentsTable inputForm" summary="신입생 원서접수 지원자 인적사항">
						<caption>
						신입생 원서접수 지원자 인적사항
						</caption>
						...
						<tbody>
							<tr>
								<th class="txtLeft">
									<label for="parent_tel1">보호자 연락처<!--<span class="star">*</span>--></label>
								</th>
								<td class="txtLeft">
									<input class="w70" id="parent_tel1" name="parent_tel1" value="" type="tel"  placeholder="보호자 연락처를 입력해주세요." maxlength="15" onKeyPress="onlyNumber(event);" />
								</td>
							</tr>
						</tbody>
					</table>
					...
					<table class="contentsTable inputForm" summary="신입생 원서접수 지원자 인적사항">
						...
						<tbody>
							<tr>
								<th>
									<label for="Imotivation">지원경로<span class="star">*</span></label>
								</th>
								<td class="txtLeft" id="ImotivationWrap">
									<div class="checkboxWarp">
										<input type="radio" name="Imotivation" data-imotivation="" value="" class="input-check" id="imot-01">
										<label for="imot-01">없음</label>
									</div>
									<div class="checkboxWarp">
										<input type="radio" name="Imotivation" data-imotivation="" value="학교설명회" class="input-check" id="imot-02">
										<label for="imot-02">학교설명회</label>
									</div>
									<div class="checkboxWarp">
										<input type="radio" name="Imotivation" data-imotivation="" value="교사 추천" class="input-check" id="imot-03">
										<label for="imot-03">교사 추천</label>
									</div>
									<div class="checkboxWarp">
										<input type="radio" name="Imotivation" data-imotivation="" value="친구.지인 추천" class="input-check" id="imot-04">
										<label for="imot-04">친구,지인 추천</label>
									</div>
									<div class="checkboxWarp">
										<input type="radio" name="Imotivation" data-imotivation="" value="온라인(카카오톡)상담" class="input-check" id="imot-05">
										<label for="imot-05">온라인(카카오톡)상담</label>
									</div>
									<div class="checkboxWarp">
										<input type="radio" name="Imotivation" data-imotivation="" value="전화상담" class="input-check" id="imot-06">
										<label for="imot-06">전화상담</label>
									</div>
									<div class="checkboxWarp">
										<input type="radio" name="Imotivation" data-imotivation="" value="인터넷 검색" class="input-check" id="imot-07">
										<label for="imot-07">인터넷검색</label>
									</div>
									<div class="checkboxWarp">
										<input type="radio" name="Imotivation" data-imotivation="" value="입시 사이트" class="input-check mr2" id="imot-08">
										<label for="imot-08">입시 사이트</label>
									</div>
									<div class="checkboxWarp">
										<input type="radio" name="Imotivation" data-imotivation="" value="SNS" class="input-check" id="imot-09">
										<label for="imot-09">SNS</label>
									</div>
									<div class="checkboxWarp">
										<input type="radio" name="Imotivation" data-imotivation="" value="포스터" class="input-check" id="imot-10">
										<label for="imot-10">포스터</label>
									</div>
									<div class="checkboxWarp">
										<input type="radio" name="Imotivation" data-imotivation="etc" value="기타" class="input-check" id="imot-11">
										<label for="imot-11">기타</label>
									</div>
									<div id="ImotivationEtcWrap" class="blind">
										<input type="text" name="ImotivationEtc" class="w100" value="" maxlength="200" placeholder="지원경로 기타 사항을 입력하세요">
									</div>
								</td>
							</tr>
							<tr>
								...
								<td class="txtLeft">
									<label>
										<input name="recommend2" id="recommend2" type="radio" value="정규면접(토)">
										<strong class="point">정규면접(토)</strong></label>
									<label>
										<input name="recommend2" id="recommend2" type="radio" value="개별면접(평일)">
										<strong class="point">개별면접(평일)</strong></label>
									<label>
										<input name="recommend2" id="recommend2" type="radio" value="기타">
										<strong class="point">기타</strong></label>
								</td>
							</tr>

                            <tr>
								<th class="txtLeft">면접 구분<span class="star">*</span></th>
								<td class="txtLeft">
									<label>
										<input name="carea" id="carea" type="radio" value="offline">
										<strong class="point">오프라인(학교방문 대면) 면접</strong></label><br>
									<label>
										<input name="carea" id="carea" type="radio" value="online">
										<strong class="point">온라인(학교 미 방문 비대면) 면접</strong></label>
								</td>
							</tr>
							...
							<tr>
								...
								<td class="txtLeft">
									<input type="text" name="recommend" class="w70" value="">
									<br>
									<span class="ml10">※ 추천인이 없을 경우 빈칸으로 두세요.</span> </td>
							</tr>
							<tr>
								<th class="txtLeft">세부상담 희망분야</th>
								<td class="txtLeft">
									<label>
										<input type="radio" name="call_request" value="1">
										<strong class="point">입시</strong></label>
									<label>
										<input type="radio" name="call_request" value="2">
										<strong class="point">진로</strong></label>
									<label>
										<input type="radio" name="call_request" value="3">
										<strong class="point">편입</strong></label>
									<label>
										<input type="radio" name="call_request" value="4">
										<strong class="point">취업</strong></label>
									<label>
										<input type="radio" name="call_request" value="5">
										<strong class="point">캠퍼스투어</strong></label>
								</td>
							</tr>
						</tbody>
					</table>
					<!--	<div class="checkBox m05">
						<ul>
							<li>개인정보 활용 정보항목<br>
								합격안내 문자서비스, 원서접수 및 상담, 각종공지, 본인확인, 고지서등을 위해 성명, 핸드폰, 이메일 주소를 활용합니다. </li>
							<li> 원서정보는 개인정보취급방침에 따라 안전하게 보호되며, 본인의 명백한 동의 없이 공개 및 제3자에게 제공되지 않습니다. </li>
							<li> 원서정보는 개인정보취급방침에 따라 안전하게 보호되며, 본인의 명백한 동의 없이 공개 및 제3자에게 제공되지 않습니다. </li>
						</ul>
					</div>
				   <div class="m05">
					    원서정보는 개인정보취급방침에 따라 안전하게 보호되며, 본인의 명백한 동의 없이 공개 및 제3자에게 제공되지 않습니다.
					</div>-->
					<div class="checkBox">
					    1. 개인정보 활용 정보항목<br>
				    ...
				    <p class="privacyCheck"><label for="privacyCk"><input type="checkbox" name="privacyCk" value="1" id="privacyCk" style="vertical-align:-2px;margin-top:10px;">개인정보 취급(처리) 방침에 동의합니다.</label>
				    </p>
					<!--<div class="privacyCheck m05">
						<input type="checkbox" name="privacyCk" value="1" id="privacyCk">
						<label for="privacyCk">개인정보 취급(처리) 방침에 동의합니다.</label>
					</div>-->
					<div class="m10 tac">본인은 한국IT전문학교에 입학하고자 지원합니다.</div>
					<div class="tac"><%=year(now)%>년 <%=month(now)%>월 <%=day(now)%>일</div>

					<div class="btnWrap"> <a href="#" class="btnReceipt blue2020 wid135"  onClick="check_theForm();">원서제출</a> </div>
				</div>
			</form>
			<%
