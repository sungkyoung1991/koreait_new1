...
if mode="modi" then
    sql = " select top 1 * from news_board where take_idx="&take_idx
...
elseif mode="reply" then
	sql = " select Top 1 title,ref,step,re_level,content from news_board where take_idx="&take_idx
	set rs = conn.execute(sql)
...
function sendit() {
	...
	if (contents.length < 10) {
		...
		alert("글내용은 최대 600,000자 이하로 작성 해야합니다.");
		CKEDITOR.instances.content.focus();
		return;
	}
...
<%
	if not rs.eof then
		do until rs.eof
%>
				<option value="<%=rs("Sname")%>" <% if rs("Sname") = bbscode then response.write "selected" %>><%=rs("Sname")%></option>
<%
			rs.movenext
		loop
	end if
%>
				<option value="통합" <% if bbscode = "통합" then response.write "selected" %>>통합</option>
				</select>       
		...
		<tr> 
			...
			<td> 
				...
				<%If filename1 <> "" then%><a href="../../common/download.asp?folder=news&file=<%=filename1%>"><%=filename1%></a><%End if%>
				<br>
				...
				<br>
				<input name="filename5" type="file" class="select" />
				<%If filename5 <> "" then%><a href="../../common/download.asp?folder=news&file=<%=filename5%>"><%=filename5%></a><%End if%>
			</td>
		</tr>
	</table>
